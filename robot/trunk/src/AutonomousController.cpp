...
void AutonomousController::CreateQueue() {
	...
	case (kTestAuto): {
		...
		DriveCommand* turndrive = new DriveCommand(robot, 0.0, 2.0, 70.0);
...
*/
	...
	case (kDriveAuto): {
		std::string bin = "/home/lvuser/bins.txt";
		PlaybackAutoCommand* bins = new PlaybackAutoCommand(robot, bin);
		firstCommand = bins;

		/*
		 *Drive forward the autozone.
		 */
		switch (autoStart) {
		case (kA): {
			break;}
	...
	case (k1ToteAuto): {
		/*
		 * Intake one tote and drive to the autozone to drop it.
		 */
		std::string bin = "/home/lvuser/bins.txt";
		PlaybackAutoCommand* bins = new PlaybackAutoCommand(robot, bin);
		firstCommand = bins;
		...
		DriveCommand* dc = new DriveCommand(robot, 0.0, 10.25, 0.0);
		AddtoQueue(dc, bins);
	...
	case (k1BinAuto): {
		/*
		 * Intake one bin and drive to the autozone to drop it.
		 */
		std::string bin = "/home/lvuser/bins.txt";
		PlaybackAutoCommand* bins = new PlaybackAutoCommand(robot, bin);
		firstCommand = bins;
	...
	case (k1Tote2BinAuto): {
		/*
		 * Intake one bin and one tote and then pick up another bin to place in the
		 * autozone and then drive to the autozone to drop the stack
		 */
		break;}
	case (k1Tote3BinAuto): {

/*  Ini - Multi-Platform INI file parser, suitable for embedded systems
 *
 ...
 *  under the License.
 *
 *  Version: $Id: Ini.c 42 2012-01-04 12:14:54Z thiadmer.riemersma $
 */
...
#include "Ini.h"
  ...
  #include <string.h>
  #include <stdlib.h>
  /* definition of TCHAR already in Ini.h */
  #define __T(s)    s
...
int strnicmp (const TCHAR * a, const TCHAR * b, int n){
	int i;
	for (i = 0; i < n; i++){
		if(toupper(a[i]) > toupper(b[i])) return 1;
		if(toupper(a[i]) < toupper(b[i])) return -1;
		if (toupper(a[i]) == toupper(b[i])) {
			if (toupper(a[i]) == 0) return 0;
		}
	}
	return 0;
}
